...
| "Re-priority" / "přepočítej Eisenhower" | `agenda-priority-review` nebo `agenda-triage` |
| "Týdenní shrnutí" / "weekly review" | `agenda-weekly-review` |
| "Retro" / "jak zlepšit systém" | `agenda-retro` |
| "Schval pending triáž" | `agenda-triage` (PENDING) |
| "Analyzuj …" / "rozbor materiálů" / "deep dive" | `agenda-analyze` → `.md` ve výstupech |
...
- Necpi vše do `daily/` — třiď podle skutečného zdroje (sembly/slack/email, pokud to tam patří)
